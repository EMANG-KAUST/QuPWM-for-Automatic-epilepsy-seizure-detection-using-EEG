...
global Levels Level_intervals Mdl_SVM_mPWM clustering
...
%% get the normal Distribution N(mu0, sigma0)
        ...
        [Levels, Level_intervals]=Set_levels_Sigma_2(k,M,mu0,sigma0);

%% Cross-Validation
subject=patient_k;
tic

[sz_fPWM, Accuracy,Avg_sensitivity,Avg_specificity,Avg_precision,Avg_gmean,Avg_f1score,Avg_AUC,Mdl_SVM_mPWM]=PWM8_Data_CrossValidation_2(X, y,CV_type_list, K,type_clf_list,clustering);
...
exec_time=toc;
   ...
   %% get the LOO performance
        ...
        cnt_inc=cnt_inc+1;
        Output_results(cnt_inc,:)=[size(X,1),M,mu0,sigma0,k,resolution,sz_fPWM,Avg_sensitivity,exec_time];
        Classifier{cnt_inc}=type_clf_list;
        
        % Get the best accuracy 
        if Acc_op<Avg_sensitivity%Accuracy%Avg_sensitivity
            PWM_op_results=[M,mu0,sigma0,k,resolution,sz_fPWM,Avg_sensitivity,exec_time];
            Acc_op=Avg_sensitivity;%Accuracy;%Avg_sensitivity;
